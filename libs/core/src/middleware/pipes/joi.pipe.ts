...
import { BadRequestError } from '@core/core/providers/error-provider';
...
import type { JoiValidationGroup } from 'joi-class-decorators';
...
import type { Constructor } from 'nestjs-joi/internal/defs';
...
export class JoiPipe implements PipeTransform {
  ...
  transform(payload: unknown, metadata: ArgumentMetadata): unknown {
    ...
    return JoiPipe.validate(
      ...
      // It is technically impossible for this to be undefined since it is explicitly assigned
  ...
  private static validate<T>(
    ...
    if (error) {
      ...
      if (Joi.isError(error)) {
        ...
        if (
          formattedMessage?.includes('must be one of') &&
          error.details[0].context?.valids?.length
        ) {
          formattedMessage =
            formattedMessage.split('must be one of')[0] +
            'must be one of ' +
            error.details[0].context?.valids.join(', ');
        }
  ...
  private parseOptions(pipeOpts?: JoiPipeOptions): ValidatedJoiPipeOptions {
    ...
    let unknownKeys = Object.keys(pipeOpts).filter(
      (k) => !JOI_PIPE_OPTS_KEYS.includes(k)
    );

    unknownKeys = unknownKeys.filter(
      (k) => !['data', 'pattern', 'context'].includes(k)
    );
  ...
  private getSchema(metadata: ArgumentMetadata): Joi.Schema | undefined {
    ...
    if (this.type) {
      // Don't return "no schema" (undefined) if a type was explicitly specified
   ...
   * inbuilt types.
   * Returning a schema can be forced to cover cases when a type has been explicitly
   * passed to JoiPipe.
   *
